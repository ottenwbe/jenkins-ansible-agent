...
docker-arm: versions.sh ## Create the ansible docker image for linux/arm/v7
	...
	docker buildx build --build-arg BASE_IMAGE_TAG=$(JENKINS_IMAGE_TAG) --build-arg ANSIBLE_VERSION=$(ANSIBLE_IMAGE_VERSION) --label "version=${ANSIBLE_IMAGE_VERSION}" --label "build_date=${BUILD_DATE}" --platform linux/arm/v7 --label "maintainer=Beate Ottenwaelder <ottenwbe.public@gmail.com>" --push -f Dockerfile.armhf -t $(REPO)/$(NAME):armv7-$(ANSIBLE_IMAGE_VERSION) .
